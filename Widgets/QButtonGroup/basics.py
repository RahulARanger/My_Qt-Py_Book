...
class test(QtWidgets.QApplication):
    def __init__(self):
        ...
        self.colorGroup.addButton(self.yellow,
                                  id=3)  # radio buttons without any group acts unison so it's better to group them
        print(self.colorGroup.buttons())
        self.fontGroup = QtWidgets.QGroupBox()  # ! avoid using the parent
        self.small, self.medium, self.large = [QtWidgets.QRadioButton(self.window, text=i, toggled=self.doresize) for i
                                               in ['Small (6)', 'Medium (9)', 'Large (20)']]
    ...
    def doresize(self):
        ...
        if '6' in text:
            self.display.setFont(QtGui.QFont(self.actual.family(), 6, self.actual.weight(), self.actual.italic()))
        elif '9' in text:
            self.display.setFont(QtGui.QFont(self.actual.family(), 9, self.actual.weight(), self.actual.italic()))
        elif '20' in text:
            self.display.setFont(QtGui.QFont(self.actual.family(), 20, self.actual.weight(), self.actual.italic()))

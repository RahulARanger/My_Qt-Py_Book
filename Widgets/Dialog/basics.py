...
class Dialog(QtWidgets.QDialog):  # !(also subclass of QtWidgets.QWidget)
    def __init__(self, parent=None):
        ...
        self.more.setWhatsThis(
            'Toggles the visiblilty of the labels content')  # ? press that '?' and then click the more button then u
...
class test(QtWidgets.QApplication):
    def __init__(self):
        super().__init__([])
        self.window = QtWidgets.QWidget(windowTitle='Normal Dialog box')
        self.app = Dialog(self.window)
        result = self.app.exec()  # return True if accept() else False for reject()
        print('Accpeted' if result else 'Rejected')

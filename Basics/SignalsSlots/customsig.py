...
class CustomWidget(QtCore.QObject):
    ...
    def __init__(self,parent):
        ...
        self.simple.connect(self.checkthis) # (bound signal(object of the unbound signal (has the emit() destroy() and connect() but the unbound doesn't have this)))
    def checkthis(self):
    ...
    def check(self,msg):
        print('used emit()') 
        self.access.emit(msg) # third emit it whereever u want
...
class test(QtWidgets.QApplication):
    def __init__(self):
        ...
        self.bt2=QtWidgets.QPushButton(self.window,text='manual',clicked=lambda x=True,y='message':self.bt.check(y))

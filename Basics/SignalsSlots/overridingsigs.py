...
class signals(QtCore.QObject):
    ...
    def manual(self,wht=None):
        if wht is None:
            a=self.showinfo.emit()
            print('$',a)
            return None # ! remains same as usual 
        elif type(wht)==bool:return self.showinfo[bool].emit(wht)
    @QtCore.pyqtSlot()
    def show1(self):
        print('showed 1')
...
class test(QtWidgets.QApplication):
    def __init__(self):
        ...
        self.bt3=QtWidgets.QTextEdit(self.window,textChanged=lambda :self.accessdata(lambda x:self.info.manual(),[]))
        self.bt4=QtWidgets.QPushButton(self.window,text='private2',clicked=lambda xy:self.accessdata(lambda x:self.info.manual(x),xy))
